...
static int lept_parse_number(lept_context *c, lept_value *v) {
    ...
    if (temp[0] == '0') temp++;
        ...
        if (!ISDIGIT1TO9(temp[0])) return LEPT_PARSE_INVALID_VALUE;
    ...
    if (*temp == 'E' || *temp == 'e') {                     //判断是否是E，e
        *temp++;
        if (*temp == '-' || *temp == '+') temp++;           //次数前面可以有正负号
        if (!ISDIGIT(*temp)) return LEPT_PARSE_INVALID_VALUE;
        while (ISDIGIT(*temp)) temp++;
    }
/*
 * strtod 功能：将一个字符串数字转化成double，并将不能被解析的部分字符串返回
 * 参数：需要转化的字符串，指向剩余字符串的字符串地址的指针
 * 返回：解析出的第一个double数字
 * 判断数字是否越界。如果数字越界，strtod会将errno置为ERANGE，而且解析出的值会返回HUDE_VAL(double)， HUDE_VALF(float)，HUDE_VALL（long double）
 */
    errno = 0;
